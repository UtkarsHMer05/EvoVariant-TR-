...
| Notebook recovery | PASS_CONTENT, CHECKPOINTS_VERIFIED, WORKER_STOPPED_BY_POLICY; LOCAL_STATUS_FIX_PENDING_SYNC | The open Colab output shows the HPO child ended with SIGTERM 15 after the policy stop; its old runner reported `EXITED_FAILURE`. The repository runner now distinguishes signal termination and the notebook monitor displays the saved status. Live notebook still needs the pushed source update; fold histories show 4 / 3 / 2, selection open. | no holdout or locked-test access | sync the pushed source before any provider-compliant resume |
