...
| Notebook recovery | PASS_CONTENT, CHECKPOINTS_VERIFIED, WORKER_STOPPED_BY_POLICY | Same Colab Drive ID retains the archived raw notebook and canonical saved 20-cell / 10-section notebook. Temporary audit cell removed; all changes saved. Fold histories are 4 / 3 / 2 and checkpoint metadata matches; the HPO worker was stopped after identifying Colab's multiple-account resource-limit restriction. | no holdout or locked-test access | wait for provider-compliant free GPU access |
