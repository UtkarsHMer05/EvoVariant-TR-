...
| TRAIN-only weighted fine-tuning | `src/evovariant_tr/adaptation/training.py`, `scripts/adaptation/train_caduceus.py` | Drive `checkpoints/caduceus_frozen_head/run.json` and checkpoint | training curves pending |
| Gene-grouped TRAIN-only HPO | `src/evovariant_tr/adaptation/folds.py`, `scripts/adaptation/run_caduceus_hpo.py` | Drive SQLite study and logs active; trial summary and selection lock pending | HPO figures pending |
