...
## Runner status fix and fine-tuning clarification, 2026-09-23

- The open Colab output shows the HPO subprocess was terminated with `SIGTERM: 15` after the provider-policy stop. The visible Hugging Face `FutureWarning` messages are not fatal. The old parent runner surfaced the expected signal as `CalledProcessError`, wrote `EXITED_FAILURE`, and printed a traceback.
- `scripts/adaptation/run_autonomous.py` now maps negative subprocess return codes to `EXITED_BY_SIGNAL` and records the child script and signal; positive nonzero exits still fail normally. The canonical notebook monitor no longer maps every nonfailure state to `EXITED_SUCCESS`. The live Colab page still shows the old saved output and has not loaded the repository fix.
- The completed frozen-head run trained only the task head. Encoder fine-tuning runs through `train_caduceus.py` when HPO selects `partial_small` (last four backbone blocks), `partial_large` (last eight), or `full_if_feasible`; trial 0 itself is frozen-head only, and trial 1 is queued as `partial_small`. No encoder fine-tuning has completed.
- CPU checks passed: `tests/adaptation` 21/21; `make validate` 739 passed, 33 deselected, 95.07% core coverage, with secret scan, Ruff, and strict mypy passing. No training, holdout, or locked-test evaluation was run for this repair.
