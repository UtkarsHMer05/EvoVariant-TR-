...
def test_flash_attn_returns():
    import torch
    import flash_attn_2_cuda as flash_attn_gpu
    q = torch.randn(1, 10, 2, 8, dtype=torch.bfloat16, device="cuda")
    k = torch.randn(1, 10, 2, 8, dtype=torch.bfloat16, device="cuda")
    v = torch.randn(1, 10, 2, 8, dtype=torch.bfloat16, device="cuda")
    cu_seqlens = torch.tensor([0, 10], dtype=torch.int32, device="cuda")
    
    returns = flash_attn_gpu.fwd(
        q, k, v, None, cu_seqlens, cu_seqlens, 10, 10, 0.0, 1.0, False, False, 1, False, None
    )
    print("LENGTH:", type(returns), len(returns))

@app.local_entrypoint()
def main():
    test_flash_attn_returns.remote()
