...
evo2_image = (
    ...
    .run_commands("python3 -c \"import re, os, shutil; path='/usr/local/lib/python3.12/site-packages/vortex/ops/attn_interface.py'; f=open(path, 'r'); text=f.read(); f.close(); text=text.replace('out, softmax_lse, S_dmask, rng_state = flash_attn_gpu.fwd(', 'res = flash_attn_gpu.fwd('); text=text.replace('out, softmax_lse, S_dmask, rng_state = flash_attn_gpu.varlen_fwd(', 'res = flash_attn_gpu.varlen_fwd('); text=text.replace('if torch.__version__ >= \\\"2.4.0\\\":', 'if False:'); text=text.replace('return out, softmax_lse, S_dmask, rng_state', '_unpack = (res[0], res[-3], res[-2], res[-1]) if isinstance(res, (tuple, list)) else (res, None, None, None)\\n    out, softmax_lse, S_dmask, rng_state = tuple((x.clone() if hasattr(x, \\'clone\\') else x) for x in _unpack)\\n    return out, softmax_lse, S_dmask, rng_state'); f=open(path, 'w'); f.write(text); pycache='/usr/local/lib/python3.12/site-packages/vortex/ops/__pycache__'; shutil.rmtree(pycache) if os.path.exists(pycache) else None\"")
