...
- The committed source is synced in the open Colab checkout at `45be47ef42461e4766cf7b8fca491afbb05fc8be`; the edited notebook is saved. Its monitor now labels the legacy SIGTERM record `EXITED_BY_SIGNAL (legacy status; see runner error)` without changing the stored record. Its final cell shows the training call path, states that encoder fine-tuning has not started, and reads the snapshot in read-only mode: trial 0 `RUNNING`, trials 1–3 `WAITING`; selection is open. No training was resumed.
