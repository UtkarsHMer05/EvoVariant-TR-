...
## Clean-restart repair, 2026-09-23

The previously generated autonomous notebook was archived as
`notebooks/archive/EvoVariant_TR_Adaptation_Autonomous_BROKEN_20260923T125938Z.ipynb`
before replacement. The canonical notebook is now generated by
`scripts/adaptation/rebuild_autonomous_notebook.py` and passes AST parsing for
all 10 code cells. Destination verification is centralized in
`scripts/adaptation/verify_destination.py`; `EvoVariantTR_original` is recorded
as optional recovery evidence and never acts as a training gate. The runner
invokes the same verifier before CUDA/model work, while launch and monitor
cells create the Drive log directory and ignore stale PID files.
