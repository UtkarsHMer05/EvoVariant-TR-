...
| 14 | Locked statistical evaluation | BLOCKED / PRE-PHASE-14 CONFIG INCOMPLETE | `research/runs/formal_cpu_20260922/phase13/pre_phase14_freeze.json` records `selection_closed=true` and frozen config hash `3ab606a1e351b536f3c32ce45da956f844904ec704963f88f1cdc256d7d77424`, but `artifacts/phase14/phase14_preflight_block_20260922.json` records missing protocol/split/provenance bindings, no serialized fitted classifier artifact for the frozen HPO model ID, no hash-bound calibrator/abstention/seed configuration, and a Phase 8 model-ID mismatch. The newly requested `$2.75` / `$2.50` allocation was not activated; no locked manifest rows or labels were read and no Modal inference started. |
...
Current control-plane checkpoint at implementation commit `1c0a81c` on `research/evovariant-tr`: no
...
bound to this HEAD. The requested Phase 14 scope also fails its freeze-completeness preflight;
the locked evaluation and downstream release promotion therefore remain fail-closed.
